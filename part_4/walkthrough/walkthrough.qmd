...
The "sign" and "verify" boxes in the diagrams are doing concrete crypto.
Our protocol uses **RSA-PSS with SHA-256** (the same scheme TLS 1.3 uses
...
flowchart TB
    subgraph SIGN_SIDE["✍️ On the server (sign)"]
        ...
        RSA1 --> SIG1["✍️ signature (256 bytes for RSA-2048)"]
    end
...
The asymmetry is what gives the security: only the holder of
`identity_private_key` can produce a signature that verifies under
`identity_public_key`. And because the digest covers *both* ephemeral
public keys, the signature is uniquely tied to this one session.
